...
impl MemberRef {
    pub fn read_member_refs(metadata: &Metadata) -> io::Result<Vec<MemberRef>> {
        ...
        for row in 0..member_ref_table.row_count {
            let class = metadata.resolve_member_ref(member_ref_table.columns[0].get_cell_u16(row) as u32).unwrap();
            let name = metadata.strings_stream.get_string_clone(member_ref_table.columns[1].get_cell_u16(row) as u32)?;
            let signature = member_ref_table.columns[2].get_cell_u16(row);
